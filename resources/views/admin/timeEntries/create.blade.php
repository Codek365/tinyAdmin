    ...
    <div class="card-body">
        <form action="{{ route("admin.time-entries.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('user') ? 'has-error' : '' }}">
                <label for="user">{{ trans('cruds.timeEntry.fields.user') }}*</label>
                <input type="number" id="user" name="user" class="form-control" value="{{ old('user', isset($timeEntry) ? $timeEntry->user : '') }}" step="1" required>
                @if($errors->has('user'))
                    <p class="help-block">
                        {{ $errors->first('user') }}
                    </p>
                @endif
                <p class="helper-block">
                    {{ trans('cruds.timeEntry.fields.user_helper') }}
                </p>
            </div>
            <div>
